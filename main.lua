...
return function(mod)
  ...
  local function iconTable(game, create)
    ...
    if create then
      options.modOptions = options.modOptions or {}
      options.modOptions[mod.id] = options.modOptions[mod.id] or {}
      options.modOptions[mod.id].icons = options.modOptions[mod.id].icons or {}
    end
    local bucket = options.modOptions and options.modOptions[mod.id]
    return bucket and bucket.icons or nil
  end
  ...
  local function loaderIconTable(game, create)
    ...
    if create then
      loader.modOptions = loader.modOptions or {}
      loader.modOptions[mod.id] = loader.modOptions[mod.id] or {}
      loader.modOptions[mod.id].icons = loader.modOptions[mod.id].icons or {}
    end
    local bucket = loader.modOptions and loader.modOptions[mod.id]
    return bucket and bucket.icons or nil
  end
  ...
  local function setIconChoice(game, key, value)
    ...
    if loader and loader.events then
      loader.events:emit("mod.options_changed",
        { mod = mod.id, key = "icons." .. key, value = stored })
    end
  end
